...
extension String {
    ...
    func findFirstGroupInRegex(regexString: String) -> String {
        let wholeRange = NSMakeRange(0, utf16.count)
        let titleRegex = try! NSRegularExpression(pattern: regexString)
        let titleResult = titleRegex.matches(in:self, range: wholeRange)
        let a = titleResult.first
        if let r = a?.range(at: 1),
           let range = Range(r, in: self) {
            return "\(self[range])".trimmingCharacters(in: CharacterSet.whitespacesAndNewlines).decodingHTMLEntities()
        } else {
            return ""
        }
    }
    ...
    func removeSubstring(_ substring: String) -> String {
        return replacingOccurrences(of: substring, with: "", options: [String.CompareOptions.caseInsensitive])
    }
    
    func trimmingCharactersRecursive(_ charString: String) -> String {
        let last = self
        
        let trimmed = trimmingCharacters(in: CharacterSet.init(charactersIn: charString))
        ...
        if (last == trimmed) {
            ...
            return trimmed.trimmingCharactersRecursive(charString)
        }
    }
}
